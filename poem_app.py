...
#st.markdown(f"{target_lines[st.session_state['page_num']][0]}<br>:gray[{base_lines[0]}]<br>{target_lines[page_num][1]}<br>:gray[{base_lines[1]}]<br>{target_lines[page_num][2]}<br>:gray[{base_lines[2]}]<br>{target_lines[page_num][3]}<br>:gray[{base_lines[3]}]",unsafe_allow_html=True )
...
if st.button("Previous Page"):
  st.session_date['page_num']=st.session_state['page_num']-1
